...
- `02-trip-create`：复用当前真实的旅行草稿和受保护封面上传；目的城市使用搜索候选为主、地图确认为辅，不写死设计图中的城市或封面。
...
- `06-trip-map`：Marker 按 Memory 时间顺序编号；连线表示记忆顺序，不代表真实导航路线。无定位 Memory 时允许以 Trip 目的城市为默认中心，但不得生成虚假点位。
...
- 旅行内页复用 `TripViewNav.vue`；地图、收藏回看和同行者复用 `TripContextCard.vue`，避免各页重复实现旅行封面、标题和日期摘要。
